...
z_values = Batry[:, 3]  # Los valores Z de la batimetría
...
# Graficar contornos
hE = ax.contourf(X, Y, Z, levels=20, cmap='jet')
...
cbar = plt.colorbar(hE, cax=cbar_ax)
...
ax.fill(urbano[:, 0], urbano[:, 1], color="k")  # Rellenar urbano con color amarillo
ax.fill(Buenav_Isla1[:, 0], Buenav_Isla1[:, 1], color="k")  # Rellenar urbano con color amarillo
...
output_path = os.path.join(output_dir, 'Batry_Buenaventura.png')
